...
class resultClass:
    def __init__(self,root): 
        ...
        #====button====#
        btn_add=Button(self.root,text="submit",font=("times new roman",15),bg="lightgreen",activebackground="lightgreen",cursor="hand2",command=self.add).place(x=300,y=420,height=35)
        btn_add=Button(self.root,text="clear",font=("times new roman",15),bg="lightgreen",activebackground="lightgray",cursor="hand2",command=self.clear).place(x=430,y=420,height=35)

        #===image===#
        self.bg_img=Image.open("images/3949116.jpg")
    ...
    def search(self):
        ...
        try:
            ...
            if row!=None:
                ...
                messagebox.showerror("Error","no error found",parent=self.root)     
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to {str(ex)}",parent=self.root)
    ...
    def add(self):
        ...
        try:
            if self.var_roll.get()=="":
                messagebox.showerror("Error","please first enter the student record",parent=self.root)
            else:
...
def add_per_column():
    ...
    cur = con.cursor()
    try:
        cur.execute("ALTER TABLE result ADD COLUMN per TEXT")
        con.commit()
        print("Column 'per' added successfully.")
